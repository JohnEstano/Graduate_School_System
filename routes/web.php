...
/*
|--------------------------------------------------------------------------
| Adviser view: Defense Requirement Details (no scheduling, no panel assignment)
|--------------------------------------------------------------------------
*/
Route::get('/adviser/defense-requirements/{defenseRequest}/details', function(DefenseRequest $defenseRequest) {
    $user = Auth::user();
    if (!$user || !in_array($user->role, ['Faculty','Adviser'])) abort(403);

    // Map only the fields needed for the adviser view
    $mapped = [
        'id' => $defenseRequest->id,
        'first_name' => $defenseRequest->first_name,
        'middle_name' => $defenseRequest->middle_name,
