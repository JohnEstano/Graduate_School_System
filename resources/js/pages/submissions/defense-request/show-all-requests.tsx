...
export default function ShowAllRequests({ defenseRequests }: { defenseRequests: DefenseRequestSummary[] }) {
    ...
    const filtered = useMemo(() => {
        ...
        return defenseRequests.filter((r) => `${r.first_name} ${r.last_name} ${r.thesis_title}`.toLowerCase().includes(search.toLowerCase()));
    ...
    return (
        <div className="">
            <div>
                <h1 className="p-2 text-xl font-bold">All defense requests</h1>
            </div>
            <Card className="rounded-sm shadow">
                ...
                <CardContent className="overflow-auto">
                    <Table className="min-w-[800px]">
                        ...
                        <TableBody>
                            {paged.map((r, idx) => (
                                <TableRow key={r.id} className={`${idx % 2 === 0 ? 'bg-muted' : ''} hover:bg-muted/50 transition-colors`}>
                                    <TableCell>
                                    ...
                                    <TableCell>
                                        <Tooltip>
                                            <TooltipTrigger asChild>
                                                <span className="inline-block max-w-xs cursor-help truncate">{r.thesis_title}</span>
                                            </TooltipTrigger>
                                        ...
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end">
                                                {(['approve', 'reject', 'needs-info'] as const).map((act) => (
                                                    <DropdownMenuItem key={act} onClick={() => handleAction(r.id, act)} disabled={loadingId === r.id}>
                                                        {loadingId === r.id ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
