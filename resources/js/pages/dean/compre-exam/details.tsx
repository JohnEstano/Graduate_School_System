...
export default function Details({ application }: { application: CompreExamApplicationSummary }) {
  ...
  async function fetchEligibility() {
    ...
    // Simulation fallback (or mix with API if keys missing). URL overrides: grades/docs/bal = 1|0
    const q = typeof window !== 'undefined' ? new URLSearchParams(window.location.search) : null;
    const getBool = (v: any): boolean | null =>
      typeof v === 'boolean' ? v : v === 1 || v === '1' ? true : v === 0 || v === '0' ? false : null;

    const gradesComplete = getBool(data?.gradesComplete ?? data?.completeGrades ?? q?.get('grades'));
    const documentsComplete = getBool(data?.documentsComplete ?? data?.completeDocuments ?? q?.get('docs'));
    const noOutstandingBalance = getBool(data?.noOutstandingBalance ?? data?.hasNoOutstandingBalance ?? q?.get('bal'));
  ...
  }, [application.id, application.school_id, application.email]);
  ...
  return (
    <div className="max-w-md w-full text-sm mx-auto">
      <div className="rounded-lg border bg-white/60 dark:bg-background p-3">
        ...
        {/* Eligibility (from API or simulated) */}
        <div className="mt-3">
