...
type Field = {
  id:string; key:string; page:number; x:number; y:number;
  width:number; height:number; type:'text'|'multiline'|'signature'; font_size?:number;
};
const KEYS = [
            ...
            <div className="relative flex-1 border p-2 overflow-auto max-h-[80vh] bg-neutral-50">
              ...
              {!loading && pdfPages[page-1] && (
                <div style={{position:'relative',width:pdfPages[page-1].width,height:pdfPages[page-1].height}}>
                  <div
                    ref={el=>{
                      if(el && pdfPages[page-1] && !el.firstChild){
                        el.appendChild(pdfPages[page-1].cloneNode(true));
                      }
                    }}
                  />
                  {fields.filter(f=>f.page===page).map(f=>(
