...
export default function DefenseRequestDetailsPage(rawProps: any) {
  ...
  useEffect(() => {
    // Only fetch templates if approved
    ...
    if (selectedTemplateId) {
      fetch(`/api/document-templates/${selectedTemplateId}`)
        ...
        .then(tpl => {
          ...
          // Pre-fill values from request
          const values: Record<string, string> = {};
          (tpl.fields || []).forEach((f: { key: string }) => {
            // Example: student.full_name
            if (f.key === 'student.full_name') {
              values[f.key] = `${request.first_name} ${request.last_name}`;
            }
            // Add more auto-fill logic as needed
            if (f.key === 'student.program') {
              values[f.key] = request.program;
            }
            if (f.key === 'request.thesis_title') {
              values[f.key] = request.thesis_title;
            }
            // ...etc for other keys
          });
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch('/api/generate-document', {
        ...
        body: JSON.stringify({
          ...
          fields: Object.fromEntries(
            templateFields.map(f => [f.key, fieldValues[f.key] ?? ''])
          )
        })
      });
      ...
      if (res.ok && data.ok && data.download_url) {
        ...
        console.error('Document generation error:', data.error || text);
      }
    } catch (err) {
      toast.error('Network or server error during document generation.');
            ...
            {request.status === 'Approved' && (
              <div className="rounded-lg border p-5 space-y-4">
                ...
                {templateFields.length > 0 && (
                  <div className="space-y-2">
                    <h3 className="text-xs font-semibold">Field Values</h3>
                    {templateFields.map(f => (
                      <div key={f.id || f.key} className="flex gap-2 items-center">
                        <label className="text-xs w-40">{f.key}</label>
                        <input
