...
import IndiviualRecord from './individual-records';
import EditStudentRecord from './edit-student-record';
  ...
  const handleDelete = () => {
    if (recordToDelete) {
      const updated = allRecords.filter(
        (record) => record.studentNo !== recordToDelete.studentNo
      );
            ...
            {records.map((record, index) => (
                <tr key={index} className="border-b hover:bg-gray-50">
                    ...
                    <button
                        ...
                        onClick={() => setShowIndividualRecord(true)}
                    >
