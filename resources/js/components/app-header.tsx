...
export function AppHeader({ breadcrumbs = [] }: AppHeaderProps) {
    ...
    const { first_name, middle_name, last_name, avatar } = auth.user;
                        ...
                        <NavigationMenu className="flex h-full items-stretch">
                            <NavigationMenuList className="flex h-full items-stretch space-x-2">
                                {mainNavItems.map((item, idx) => (
                                    <NavigationMenuItem key={idx} className="relative flex h-full items-center">
                                        <Link
                                            href={item.href}
                                            className={cn(
                                    ...
                                    <Tooltip>
                                        <TooltipTrigger>
                                            <a
                                                ...
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="group text-accent-foreground ring-offset-background hover:bg-accent hover:text-accent-foreground focus-visible:ring-ring ml-1 inline-flex h-9 w-9 items-center justify-center rounded-md p-0 font-medium transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50"
                                            >
                                                {/* ← and here, too */}
                                ...
                                <Button variant="ghost" className="size-10 rounded-full p-1">
                                    <Avatar className="size-8 overflow-hidden rounded-full">
                                        ...
                                        <AvatarFallback className="rounded-lg bg-neutral-200 text-black dark:bg-neutral-700 dark:text-white">
                                            {getInitials(first_name, middle_name, last_name)}
                                        </AvatarFallback>
                                    </Avatar>
                                </Button>
                            </DropdownMenuTrigger>
